...
# add mode and permissions for files we added above
  ...
  yum install -y centos-release-scl-rh grubby && \
  yum autoremove -y && \
  yum remove -y grubby && \
  yum upgrade -y && \
...
# add our user and group first to make sure their IDs get assigned consistently, regardless of whatever dependencies get added
  ...
  yum install -y mercurial subversion openssh-clients bash unzip procps rh-git227-git-core sysvinit-tools coreutils curl && \
